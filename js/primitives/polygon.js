class Polygon {
    ...
    static break(poly1, poly2) {
        ...
        for (let i = 0; i < segs1.length; i++) {
            for (let j = 0; j < segs2.length; j++) {
                const int = getIntersections(segs1[i].p1, segs1[i].p2, segs2[j].p1, segs2[j].p2);
    ...
    intersectsPoly(poly) {
        for (let s1 of this.segments) {
            for (let s2 of poly.segments) {
                if (getIntersections(s1.p1, s1.p2, s2.p1, s2.p2)) {
                    return true;
                }
            }
        }
    ...
    containsPoint(point) {
        ...
        for (const seg of this.segments) {
            const int = getIntersections(outerPoint, point, seg.p1, seg.p2);
            if (int) {
                intersectionsCount++;
            }
        }
